...
export async function PATCH(request: Request, { params }: Props) {
  ...
  const isVisible = getBoolean(formData, 'isVisible')
  const sortOrder = Number(getText(formData, 'sortOrder') || 1000)
  const image = formData.get('image')
  const buildingImage = formData.get('buildingImage')
  const removeBuildingImage = getBoolean(formData, 'removeBuildingImage')
  ...
  try {
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error ? error.message : 'Gagal mengubah produk katalog.',
      },
      { status: 500 }
    )
  }
...
export async function DELETE(request: Request, { params }: Props) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error ? error.message : 'Gagal menghapus produk katalog.',
      },
      { status: 500 }
    )
  }
